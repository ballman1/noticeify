...
const GEO_LOOKUP_ENABLED = (process.env.GEO_LOOKUP_ENABLED || 'true') === 'true';
const GEO_LOOKUP_FAILURE_THRESHOLD = parseInt(
  process.env.GEO_LOOKUP_FAILURE_THRESHOLD || '3',
  10
);
const GEO_LOOKUP_COOLDOWN_MS = parseInt(
  process.env.GEO_LOOKUP_COOLDOWN_MS || '60000',
  10
);
...
async function geoLookup(ip) {
  ...
  if (Date.now() < geoDisabledUntilMs) {
    return { countryCode: null, regionCode: null };
  }
