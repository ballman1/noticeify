...
async function stopScannerWorker() {
  ...
  while (isProcessing && Date.now() - start < timeoutMs) {
    // wait for active processing loop to finish
  ...
  function getScannerWorkerMetrics() {
    return { ...metrics, workerStarted, isProcessing };
  }
  ...
  return {
    ...
    _processNextPendingScan: processNextPendingScan,
  };
}
...
const getScannerWorkerMetrics = defaultWorker.getScannerWorkerMetrics;
